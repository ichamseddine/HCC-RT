...
def plot_roc(tprs, mean_fpr, aucs, endpoint, clfname, pathsave):
    ...
    auc_hi = np.percentile(aucs, 97.5, axis = 0)
    ax.plot(mean_fpr, mean_tpr, color='k',lw=2, alpha=.8)
    
    tprs_upper = np.percentile(tprs, 2.5, axis = 0)
    tprs_lower = np.percentile(tprs, 97.5, axis = 0)
...
def plot_prec_recall(precs, mean_recall, aps, endpoint, clfname, pathsave, y_true):
    ...
    prec_up = np.percentile(precs, 97.5, axis = 0)
    prec_lo = np.percentile(precs, 2.5, axis = 0)
    ap_up = np.percentile(aps, 97.5, axis = 0)
    ap_lo = np.percentile(aps, 2.5, axis = 0)
